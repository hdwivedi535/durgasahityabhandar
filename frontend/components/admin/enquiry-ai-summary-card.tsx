import type { EnquiryAiSummaryDto } from '@dsb/shared';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader } from '@/components/ui/card';

interface EnquiryAiSummaryCardProps {
...
export function EnquiryAiSummaryCard({
  summary,
  canGenerate,
  generating,
  error,
  onGenerate,
}: EnquiryAiSummaryCardProps) {
    ...
    <Card>
      <CardHeader className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted">Internal only. Generated from existing enquiry data.</p>
        </div>
        {canGenerate && (
          <Button
            type="button"
          ...
          <div>
            ...
            {summary?.stale && (
              <p className="mt-2 text-xs text-amber-700">
                The enquiry has changed since this summary was generated.
              </p>
            )}
          </div>
        ) : (
